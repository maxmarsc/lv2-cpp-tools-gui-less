...
namespace PAQ {
  ...
  RDFTerm* TurtleParser::do_datatypeString(iter_t iter) {
    ...
    iter_t child = iter->children.begin();
    assert(child->value.id() == parser_id(&quotedString));
    RDFTerm* data = do_quotedString(child);
    assert(data);
    // Skip two nodes ahead. We don't care about the "^^" string, and
    // we know that there are exactly 3 children.
    std::advance(child, 2);
    assert(child->value.id() == parser_id(&resource));
    RDFTerm* type = do_resource(child);
    assert(type);
    data->datatype = type;
    return data;
  }
